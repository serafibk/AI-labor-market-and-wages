...
seed = 101 # use for reproducibility of initial simulations to debug 
...
class Worker:
    ...
    def _p_seek_info(self):
        ...
        '''
        #self.optimistic == 1 -- if it is an explicit property of worker to use ambiguous mass a certain way

        if self.mass_mvpt_useful > self.mass_mvpt_not_useful: # count ambiguous mass towards usefulness of the tool, ambiguity reinforces certain beliefs
    ...
    def _seek_mass_general_update(self, f_useful, f_not_useful, f_ambiguous):
        
        if f_useful + f_not_useful + f_ambiguous != 1:
            print(f"Error, mass is not bma: <{f_useful}, {f_not_useful}, {f_ambiguous}>")
            return 

        self.n_seeks = self.n_seeks + 1 # tracks 't'

        self.mass_mvpt_not_useful = (self.n_seeks*self.mass_mvpt_not_useful + f_not_useful) / (self.n_seeks + 1)
        self.mass_mvpt_useful = (self.n_seeks*self.mass_mvpt_useful + f_useful) / (self.n_seeks + 1)
        self.mass_mvpt_ambiguous = (self.n_seeks*self.mass_mvpt_ambiguous  + f_ambiguous) / (self.n_seeks + 1)
    ...
    def mvpt_information_seeking(self,mvpt,max_switches):
        ...
        def _evidence_1_processing(l_hat, u_hat, wage, tau=0.01):
            ''' returns <f_useful, f_not_useful, f_ambiguous'''

            if wage < l_hat:
                # print(f"useful: {1-wage}")
                return 1-(u_hat-l_hat)/l_hat * wage,0,  (u_hat-l_hat)/l_hat * wage
            elif wage < u_hat:
                # print(f"not useful: {l_hat}")
                # return 0.5-(u_hat-l_hat)*0.5,(u_hat-l_hat)*0.5,0.5 # I think this causes beliefs to always converge to uncertain
                return 1-(u_hat-l_hat),0,(u_hat-l_hat)
            else: # some use to sharing, but not much, decreases as u_hat increases (?)
                # print(f"not useful: {1-min(tau,1-u_hat)}")
                return  0,1-min(tau,1-u_hat) , min(tau,1-u_hat)

        f_useful, f_not_useful, f_ambiguous = _evidence_1_processing(mvpt.l_hat, mvpt.u_hat, self.wage)
        self._seek_mass_general_update(f_useful, f_not_useful, f_ambiguous)
    ...
    def offer_update(self,m_hat = None):
        ...
        def _evidence_2_processing(m_hat, wage, tau=0.01):
            ''' returns <f_useful, f_not_useful, f_ambiguous>'''

            if wage < m_hat:
                return 1-wage,wage, 0
            else: # there was some use to sharing, but not much, decreases as m_hat increases (?)
                return min(tau,1-m_hat), 1-min(tau,1-m_hat),0


        if m_hat is not None: # update belief with more evidence and possibly create an offer

            if m_hat > self.reservation_wage: # create an offer using mvpt only if it exceeds reservation wage
                ...
                f_useful, f_not_useful, f_ambiguous = _evidence_2_processing(m_hat, self.reservation_wage)
                self._seek_mass_general_update(f_useful, f_not_useful, f_ambiguous)
        else:
...
class Market:
    ...
    def __init__(self, N_f, N_w, C, f, a, s, b_k, sd_cap, i_p,o_o_c, lam_H, lam_L, J, m_useful, m_not_useful, m_ambiguous,i_w_l_p,i_w_u_p):
        ...
        self.workers = [Worker(s,f,a,None,mass_le_m_hat=m_useful, mass_g_m_hat=m_not_useful, mass_ambiguous=m_ambiguous) for i in range(N_w)] # workers identical up to initial wage and initial optimism (to be set after wage set)
        ...
        # control initial high wage and low wage 
        # e.g., i_w_l_p := l_hat is (approx) this percentile of initial wage distribution 

        ordered_wages = sorted([w.wage for w in self.workers])
        # assert(i_w_l_p>0,"Error: i_w_l_p proportion must be strictly positive for indexing")
        # assert(i_w_u_p>0, "Error: i_w_u_p proportion must be strictly positive for indexing")
        i_w_l = ordered_wages[int(i_w_l_p*N_w)-1] # index s.t. i_w_l_p proportion are below that wage
        i_w_l_idx = self.workers.index([w for w in self.workers if w.wage == i_w_l][0]) # choose a worker with that wage
        i_w_u = ordered_wages[int(i_w_u_p*N_w)-1] # index s.t. i_w_u_p proportion are below that wage
        i_w_u_idx = self.workers.index([w for w in self.workers if w.wage == i_w_u][0]) # choose a worker with that wage
        # print(f"Lower wage is {i_w_l}, realized at worker index {i_w_l_idx}")
        # print(f"Upper wage is {i_w_u}, realized at worker index {i_w_u_idx}")

         # initializing mvpt after workers have an initial wage
        initial_worker_pool = set(gen.permutation([w for w in self.workers if w.wage<= i_w_u and w.wage >= i_w_l])[:int(N_w *i_p)]) # grab i_p proportion of initial workers randomly for MVPT
        # ensure upper and lower bound in the pool
        initial_worker_pool.add(self.workers[i_w_l_idx])
        initial_worker_pool.add(self.workers[i_w_u_idx])
        print(f"Initial pool wage range: [{min([w.wage for w in initial_worker_pool])},{max([w.wage for w in initial_worker_pool])}]")

        
        self.mvpt = MVPT(initial_worker_pool,N_w,sd_cap) # initial worker pool is exactly the optimistic pool
    ...
    def _hire_initial_workers(self, worker_assignment):
        ...
        for i,f in enumerate(self.firms):
            for j,w in enumerate(worker_assignment[i]):
                ...
                wage = gen.beta(a=50,b=49) # somewhat skewed towards 1
    ...
    def market_time_step(self, T, t):
        ...
        # phase 3 - update mvpt -- NOW this is phase 2, update mvpt for NEXT round, it's already been used for this round
        self.mvpt.update_mvpt()
    ...
    def negotiation_and_belief_update(self):
        ...
        def _evidence_3_processing(new_wage, starting_wage):
            ''' returns <f_useful, f_not_useful, f_ambiguous'''

            if new_wage > starting_wage:
                return 1,0,0
                # return new_wage-starting_wage, 0, 1-(new_wage-starting_wage)
            else:
                return 0,1,0
        ...
        for w in gen.permutation(self.workers): # randomize sequence of negotiations so that the same workers are not getting consistenly filtered out due to capacity
            ...
            if len(firm.workers)>= firm.capacity:
                ...
                # f_useful, f_not_useful, f_ambiguous = _evidence_3_processing(w.offer, w.wage)
                # w._seek_mass_general_update(f_useful, f_not_useful, f_ambiguous)
            ...
            if outcome == -1:
                ...
                f_useful, f_not_useful, f_ambiguous = _evidence_3_processing(w.wage, w.wage)
                w._seek_mass_general_update(f_useful, f_not_useful, f_ambiguous)
                failed_mvpt_negotiations = failed_mvpt_negotiations+ 1
            else:
                f_useful, f_not_useful, f_ambiguous = _evidence_3_processing(outcome, w.wage)
                w._seek_mass_general_update(f_useful, f_not_useful, f_ambiguous)
