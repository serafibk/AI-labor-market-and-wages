...
from labor_market_social_learning_classes import Market
...
if __name__ == "__main__":
    ...
    # variable attributes -- keeping record
    ipps = [1,0.9,0.75,0.5,0.25,0.1,0]
    ipmwcs = [1,0.9,0.7]
    optimism_adjustment = [5,10,100]
    pessimism_adjustment = [5,10,100]

    initial_mass = [(0.15,0.15,0.7)] #,,(1/4,1/4,1/2),,(0.05,0.05,0.9)(1/2,1/2,0),(0.2,0.2,0.6),(0.1,0.1,0.8)(1/2,1/2,0),(1/3,1/3,1/3),(0.2,0.2,0.6)
    initial_wage_signal = [(0.1,0.25),(0.75,0.9),(0.1,1)]#(0.5,1),(0.75,1)(0.5,0.9)(0.1,0.5)

    settings_to_test =  [(0.5,1,0,0)]#[(0,1,100,5), (0.1,1,100,5), (0.25,0.7,100,5), (0.5,1,10,10), (0.75,1,5,10),(0.9,0.9,5,10),(1,1,5,5)]

    N = 1 # number of simualations to run with the above parameters
    save = True # save plots?


    # for initial_pool_proportion in ipps:
    #     for initial_pool_max_wage_cap in ipmwcs:
    #         for o_adj in optimism_adjustment:
    #             for p_adj in pessimism_adjustment:

    initial_pool_proportion= 0.5
    initial_pool_max_wage_cap = 1
                
    for m_u,m_n_u, m_a in initial_mass:
        for i_w_l_p, i_w_u_p in initial_wage_signal:
            ...
            for n in range(N):
                ...
                # else:
                #     save = False
                print(f"SETTING: <m_u,m_n_u,m_a> = <{m_u:.2f},{m_n_u:.2f},{m_a:.2f}>")
                market = Market(N_f = N_firms, N_w=N_workers,C=firm_capacity, f=failure_threshold, a=acceptance_threshold, s=search_threshold,b_k = benchmark_proportion,sd_cap=sd_cap,i_p=initial_pool_proportion,o_o_c=outside_offer_cutoff, lam_H=lam_high, lam_L=lam_low, J=J,m_useful = m_u,m_not_useful=m_n_u, m_ambiguous=m_a,i_w_l_p=i_w_l_p,i_w_u_p=i_w_u_p)
                ...
                worker_mvpt_useful = [[] for w in range(N_workers)]
                worker_mvpt_uncertainty = [[] for w in range(N_workers)]
                worker_mvpt_not_useful = [[] for w in range(N_workers)]
                # worker_mvpt_trust = [[] for w in range(N_workers)]
                for i in range(N_workers):
                        worker_mvpt_useful[i].append(market.workers[i].mass_mvpt_useful)
                        worker_mvpt_not_useful[i].append(market.workers[i].mass_mvpt_not_useful)
                        worker_mvpt_uncertainty[i].append(market.workers[i].mass_mvpt_ambiguous)
                ...
                for t in tqdm(range(T)):
                    ...
                    if t % 50 == 0:
                        print(f"market: {market.firms[0].market_pay_belief[0]}, {market.firms[0].market_pay_belief[1]}, {market.firms[0].market_pay_belief[2]}")
                        print(f"MVPT: {market.mvpt.l_hat}, {market.mvpt.m_hat}, {market.mvpt.u_hat} (Data pool size = {len(market.mvpt.data_pool)/N_workers})")
                ...
                if save:
                    plt.savefig(f"simulation_results/seed=101/i_p_{initial_pool_proportion}_i_w_l_p_{i_w_l_p}_i_w_u_p_{i_w_u_p}_i_m<{m_u:.2f},{m_n_u:.2f},{m_a:.2f}>_mvpt_negotiations_seed={seed}_n={n}.png")
                    # plt.savefig(f"simulation_results/i_p_{initial_pool_proportion}_i_p_w_c_{initial_pool_max_wage_cap}_o_adj_{o_adj}_p_adj_{p_adj}_{n}_mvpt_negotiations_seed={seed}.png")
                ...
                for k in range(0,50,1):
                    plt.plot(worker_mvpt_useful[k],label=f"mvpt_useful", linestyle="solid",color="blue")
                    plt.plot(worker_mvpt_not_useful[k],label=f"mvpt_not_useful", linestyle="solid",color="red")
                    plt.plot(worker_mvpt_uncertainty[k],label=f"mvpt_ambiguous", linestyle="solid",color="purple")
                    plt.legend(title="Worker characteristics")
                    plt.xlabel("Time")
                    plt.ylabel("Worker beliefs")
                    plt.title(f"worker index {k} with wage delta {(market.workers[k].wage-initial_wages[k]):.4f}")
                    plt.ylim((0,1))
                    if save:
                        plt.savefig(f"simulation_results/seed=101/i_p_{initial_pool_proportion}_i_w_l_p_{i_w_l_p}_i_w_u_p_{i_w_u_p}_i_m<{m_u:.2f},{m_n_u:.2f},{m_a:.2f}>_worker={k}_seed={seed}_n={n}.png")
                ...
                plt.legend()
                if save:
                    plt.savefig(f"simulation_results/seed=101/i_p_{initial_pool_proportion}_i_w_l_p_{i_w_l_p}_i_w_u_p_{i_w_u_p}_i_m<{m_u:.2f},{m_n_u:.2f},{m_a:.2f}>_mvpt_values_over_time_seed={seed}_n={n}.png")
                    # plt.savefig(f"simulation_results/i_p_{initial_pool_proportion}_i_p_w_c_{initial_pool_max_wage_cap}_o_adj_{o_adj}_p_adj_{p_adj}_{n}_mvpt_values_over_time_seed={seed}.png")
                ...
                if save:
                    plt.savefig(f"simulation_results/seed=101/i_p_{initial_pool_proportion}_i_w_l_p_{i_w_l_p}_i_w_u_p_{i_w_u_p}_i_m<{m_u:.2f},{m_n_u:.2f},{m_a:.2f}>_mvpt_pool_size_seed={seed}_n={n}.png")
                    # plt.savefig(f"simulation_results/i_p_{initial_pool_proportion}_i_p_w_c_{initial_pool_max_wage_cap}_o_adj_{o_adj}_p_adj_{p_adj}_{n}_mvpt_pool_size_seed={seed}.png")
                ...
                plot_attribute_distribution_market(market,"wage",N_workers,initial_counts_bins_market[0],initial_counts_bins_market[1],initial_pool_proportion,seed,save,n,initial_pool_max_wage_cap,outside_offer_cutoff,failure_threshold,lam_high,lam_low,J,c_b_O=c_b_O, c_b_P=c_b_P, c_b_B = c_b_B,m_u=m_u,m_n_u=m_n_u,m_a=m_a,i_w_l_p=i_w_l_p,i_w_u_p=i_w_u_p)
